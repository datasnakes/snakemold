...
Preparing your Fork
^^^^^^^^^^^^^^^^^^^
1. Hit 'fork' on Github, creating e.g. ``yourname/dstoolbox``.
2. Clone your project: ``git clone git@github.com:yourname/dstoolbox``.
3. Create a branch: ``cd dstoolbox; git checkout -b foo-the-bars 1.3``.

Development Mode Installation
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
1. Change to the project repository on your machine: ``cd dstoolbox``
